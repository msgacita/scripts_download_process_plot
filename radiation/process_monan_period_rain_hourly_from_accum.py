...
def check_missing_hours(time_index):
    """
    ...
    time_index : pd.DatetimeIndex
        Time index from the files
    
    Returns
    -------
    missing_hours : list
        List of missing hours as datetime objects
    """
    if len(time_index) == 0:
        return []
    
    # Create expected hourly time range from first to last timestamp
    ...
    # Find missing times
    missing = expected_times.difference(time_index)
    
    if len(missing) > 0:
        print(f"\n[WARNING] Found {len(missing)} missing hour(s):")
        for dt in missing:
            print(f"  - {dt.strftime('%Y-%m-%d %H:%M')}")
    else:
        print(f"\n[INFO] No missing hours detected. All {len(time_index)} hours are consecutive.")
...
def compute_hourly_from_accum(accum: xr.DataArray, var: str):
    ...
    if "units" in accum.attrs:
        myunits = accum.attrs["units"]
    # Hourly diffs
    diff = accum.diff("Time")  # (Time-1, nCells), in mm
    # Clean diffs: values in [-neg_eps, 0) -> 0; values < -neg_eps -> NaN
    diff_clean = diff.copy()
    diff_clean.attrs.update(long_name=f"Hourly {var} from diff accum)", units=myunits)
...
def main():
    ...
    # Extract year and month from init tag for output filename (format: YYYYMMDDHH)
    init_dt = pd.to_datetime(args.init, format="%Y%m%d%H")
    year = init_dt.year
    month_abbr = init_dt.strftime("%b")

    # 1) Find existing files (first N hours based on --length)
    files = list_files(diag_root, args.init, args.length, args.exp)
    if not files:
        raise SystemExit(f"No files found for initialization {args.init} with the expected pattern.")

    # 2) Build time index
    t_idx, paths = build_time_index(files)
    print(f"[INFO] Found {len(paths)} files from {t_idx[0]} to {t_idx[-1]}")
    
    # 2b) Check for missing hours
    check_missing_hours(t_idx)
    ...
    # 5) Force computation to close input file handles before writing
    print(f"[INFO] Computing hourly data (this may take a moment)...")
    hourly = hourly.compute()
    print(f"[INFO] Computation complete.")
